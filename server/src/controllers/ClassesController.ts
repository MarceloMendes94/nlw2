...
export default class ClassesController {
    async index(req:Request,res:Response){
        ...
        return res.status(400).json(classes)
    }
    ...
    async create(req:Request,res:Response){
        ...
        try{
            ...
            const classSchedule = schedule.map((scheduleItem:scheduleItem)=>{ 
                return{ 
                    class_id:classId,/*Diego safado botando bug no meu código */
            ...
            trx.rollback();
            return res.status(400).json({
                error:'Unexpected when create clas'
            })
        }
        return res.status(201).send("tudo cadastrado com sucesso")
    }
}
